...
# Define a set of known errors that GDAL warp can produce
known_gdal_errors = {
    # If this error happens, we did not get any valid output from the warp
    # command, so terminate with error/exception
    'ERROR 1': {
        'sub_match': "failed to transform",
        'allowed': False,
        'exception_str': "GDAL Warp failed to transform"
    },
    # This error happens all the time because we are using the ENVI format
    # and GDAL doesn't know that you can actually specify a no data value
    # for ENVI format
    'ERROR 6': {
        'sub_match': "SetNoDataValue() not supported",
        'allowed': True,
        'exception_str': None
    }
}
...
def gdal_error_raise_exception(line):
    '''
    Description:
        Process the line through all known GDAL errors and either raise an
        exception or continue to process, because the error is not
        significant.
    '''

    found = False
    for key in known_gdal_errors:
        obj = known_gdal_errors[key]
        if ((key in line) and (obj['sub_match'] in line)):
            if obj['allowed']:
                found = True
            else:
                raise Exception(obj['exception_str'])

    if not found:
        ''' This is a catch-all for any other errors.  If this happens, we do
            not know what the error is or if it effects the output product, so
            terminate with error and then modify the known_gdal_errors
            dictionary to accomodate the specifics of the error '''
        raise Exception("Unknown GDAL Warp failure")
...
def warp_image(source_file, output_file,
    ...
    try:
        ...
        if len(output) > 0:
            logger.info(output)
            o_split = output.split('\n')
            for line in o_split:
                if ("ERROR " in line):
                    gdal_error_raise_exception(line)
